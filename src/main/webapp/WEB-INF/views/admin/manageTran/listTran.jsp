            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title"><b>거래관리</b></h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                  <table id="memberTable" class="table table-bordered table-striped">
                    ...
                    <tbody>
                      ...
                      <c:forEach var="tranList" items="${tranList}">
                        <tr>
                          <td style="text-align:center;">${number}</td>
                          <td style="text-align:center;"><img src="/product_upload/${tranList.file1}" style="width: 50px; height: 50px;"/></td>
                          <td>${tranList.odid}</td>
                          <td>${tranList.buyer}</td>
                          <td>${tranList.dealdate}</td>
                          <td>${tranList.trandate}</td>
                          <td><fmt:formatNumber value="${tranList.payprice}" pattern="#,##0원"/></td>
                          <td>${tranList.transtatus}</td>
                          <td>${tranList.paymethod}</td>
                          <td>
                          	<c:if test="${tranList.transtatus == '입금대기'}">
                              <button type="button" class="btn btn-primary btn-ok" onclick="popup('/admin/manageTran/updateTran?tranid=${tranList.tranid}&session_cart=${tranList.cart}&session_id=${tranList.mid}')">거래승인</button>
                            </c:if>
                          </td>
                        </tr>
