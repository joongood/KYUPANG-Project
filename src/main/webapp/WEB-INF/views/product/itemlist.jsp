...
<div class="category-bar">
    <ul id="category-list">
        ...
        <c:forEach var="category1" items="${cateList}">
            <!-- cid가 2자리인 경우만 대분류로 간주하여 출력 -->
            <c:if test="${fn:length(category1.cid) == 2}">
                <li>
                    <a href="#" class="category-link" data-cid="${category1.cid}">${category1.cname}</a>
                    <div class="sub-categories">
                        <ul id="sub-category-${category1.cid}" class="sub-category-list">
                            <!-- 해당 대분류에 속하는 하위 중분류 카테고리 출력 -->
                            <c:forEach var="category2" items="${cateList}">
                                <!-- category2가 category1의 하위 중분류인 경우에만 출력 -->
                                <c:if test="${fn:startsWith(category2.cid, category1.cid) and fn:length(category2.cid) == 4}">
                                    <li>
                                        <a href="#" class="sub-category-link" data-cid="${category2.cid}">${category2.cname}</a>
                                    </li>
                                </c:if>
                            </c:forEach>
                        </ul>
        ...
        <div class="row">
           ...
           <c:choose>
              <c:when test="${empty productList}">
                 <div class="col-md-12">
                        <p style="text-align: center; font-size: 20px; color: red;">상품이 없습니다.</p>
                    </div>
                </c:when>
                <c:otherwise>
                  <!-- 기존 상품 목록 반복문 -->
                          ...
                          <div class="col-md-4 col-sm-3">
                              <div class="single-shop-product">
                                  <form method="post" action="/cart/addcart">
                                      ...
                                      <input type="hidden" class="product-upper" value="${number }">
                                      <input type="hidden" name="page" value="${pageMaker.criteria.page != null ? pageMaker.criteria.page : 1}">
                                      <input type="hidden" id="cid" name="cid" value="${product.maincate}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function navigateToCategory(cid) {
        ...
        // 새로운 cid 추가
        currentURL.searchParams.append('cid', cid);
